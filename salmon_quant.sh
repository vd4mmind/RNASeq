...
/path_to/softwares/time -f "$tf" $SALMON quant -p 8 -i $SALMON_INDEX -g $TX2GENE -l ISR -1 <(zcat $floc/$s/*R1*.fastq.gz) -2 <(zcat $floc/$s/*R2*.fastq.gz) -o /path_to/vdas/client/RNA-Seq/output/hg19_salmon_out/$s2.quant
done
